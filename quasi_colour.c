...
/* --- functions added by S J King zh84@lycos.com --- */
double theta();
double phi();
double rcolour();
double gcolour();
...
/* --- Functions declared by S J King here follow --- */
double phi(double n)
{
	return 2*PI*n;
}
...
double theta(double n)
{
	double curPhi;
	curPhi = phi(n);
	return (PI/2)*sin(3*curPhi) + PI/2;
}

double rcolour(double n)
{
	double curPhi;
	double curTheta;
	curPhi = phi(n);
	curTheta = theta(n);
	return 0.5*sin(curTheta)*cos(curPhi) + 0.5;
}

double gcolour(double n)
{
	double curPhi;
	double curTheta;
	curPhi = phi(n);
	curTheta = theta(n);
	return 0.5*sin(curTheta)*sin(curPhi) + 0.5;
}

double bcolour(double n)
{
	double curTheta;
	curTheta = theta(n);
	return 0.5*cos(curTheta) + 0.5;
}

/* --- End of functions declared by S J King --- */
	...
	if ((dx<1.3)&&(dy<1.0)&&(dx>0)&&(dy>0))  {		/* in window */
		...
		if (plotflag<1)  {
			...
			if (plotflag==2)  {
				...
				if (fillon==1)  {
					printf("g\n");
/* --- Formerly:			printf("%.1f sg\n",color); 	--- */
/* --- Here begin revisions by S J King 				--- */
/* --- Set up the three colour components. 				--- */
					curRed = rcolour(color);
					curGreen = gcolour(color);
					curBlue = bcolour(color);
/* --- Print their values out. 						--- */
					printf("%.2f %.2f %.2f sc\n", curRed, curGreen, curBlue);
/* --- Here end revisions by S J King 					--- */
					printf("fill\n");
					printf("h\n");
				}
...
printf("/sc {setrgbcolor} bind def\n"); /* --- this line added by S J King --- */
